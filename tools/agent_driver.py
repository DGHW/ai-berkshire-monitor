...
【独立检验（防同模型偏见）】四个视角必须各自独立推导，不得简单延续旧报告结论：
- 每个视角至少提出一个与旧报告不同的独立观察点（新数据/新角度/反方证据），无差异也要说明"独立复核后确认"
- 不要互相引用其他视角的估值结果作为自己的输入；各自独立估值后再交叉对比
- 对旧报告中的关键数字（PE/ROE/增速），须重新核算而非直接沿用
